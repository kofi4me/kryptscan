...
3. User enters the approved target and service-specific fields.
4. KryptNet validates account, target authorization, rate limits, and safe-use controls. Payment gating can be enabled later when the commercial workflow is ready.
5. KryptNet profiles the target and generates an assessment-specific test plan.
...
With this setting, both Vulnerability Assessment and Ethical Pen-Testing run through the bundled KryptNet toolkit engine. The frontend remains simple, while the backend orchestrates Nuclei, Nmap, OWASP ZAP, Nikto, SSLyze, testssl.sh, Amass, Subfinder, Trivy, httpx, Naabu, dnsx, Katana, Feroxbuster, ffuf, Gobuster, wafw00f, WhatWeb, Semgrep, Gitleaks, Grype, Checkov, Prowler, ScoutSuite readiness, cloud checks, and AI triage where configured.
